...
if (isset($_POST['submit'])) {
    ...
    // Update the tutorial
    $updateSql = "UPDATE blog SET Title = ?, Description = ?, File = ? WHERE Id = ?";
    $stmt = $conn->prepare($updateSql);
    $stmt->bind_param("sssi", $title, $description, $fileName, $id);

    if ($stmt->execute()) {
        echo "<script>alert('Blog updated successfully.'); window.location.href='mentor_view_blog.php';</script>";
    } else {
...
<div class="container text-center mt-5">
    <div class="row">
        <div class="col">
            <p class="text-primary fs-5">Update Blog</p>
